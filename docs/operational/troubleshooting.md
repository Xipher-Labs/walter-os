...
| Walter-Bridge (LiteLLM) returns 401 Unauthorized | `LITELLM_MASTER_KEY` in `.env.local` does not match what the container was started with, or the key has been rotated | `docker compose down litellm && docker compose up -d litellm` after correcting the key; verify with `curl -H "Authorization: Bearer $KEY" https://llm.your-domain/health` |
...
| Headscale node enrollment returns HTTP 500 and logs `capability version must be set` | Tailscale client capability-version drift against the pinned Headscale release | Do not rely on Headscale for break-glass during the outage. Use the Hetzner Cloud Firewall SSH allow-list recovery path, then follow `setup/walter-host/services/headscale/RUNBOOK.md` before pinning clients or changing the Headscale image. |
