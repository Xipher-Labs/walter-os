...
- **Promtail can also read Walter-OS audit-chain rows.** Audit telemetry is opt-in at the mount layer: the base Promtail config contains the `walter-audit-chain` scrape job, and the default compose stack does not mount `/var/log/walter-audit`, so hosts without an audit directory do not silently create one. To enable it, create the audit directory as the Walter operator and start with the override: `docker compose -f setup/walter-host/services/observability/compose.yml -f setup/walter-host/services/observability/compose.audit.yml up -d`. The override mounts `${WALTER_AUDIT_DIR:-/home/walter/.config/walter-os/audit}` read-only at `/var/log/walter-audit`; the scrape job reads `/var/log/walter-audit/chain-*.jsonl` with static Loki labels `job=walter-audit-chain`, `app=walter-os`, `kind=audit-chain`, and `host=walter-os`. The bind uses `create_host_path: false`, so Docker fails loudly instead of silently creating a root-owned host path. Set `WALTER_AUDIT_DIR` before running this compose override when Walter uses a non-default config directory; it should point at the host audit directory itself, not its parent. Promtail stores scrape positions under `/promtail/promtail-positions.yaml` on the persisted observability volume, parses the JSON row's `ts` field as the Loki timestamp using `RFC3339Nano`, then forwards the JSONL line as-is; Grafana panels start from the static labels and use targeted LogQL parsing only for derived fields.
- **Grafana provisions the audit dashboard as `walter-audit`.** The dashboard uses the provisioned Loki datasource UID `loki` and starts from the low-cardinality selector `{app="walter-os", kind="audit-chain"}` for each panel. Session IDs are not promoted to Loki labels; the session panel counts rows with a top-level `session_id` using LogQL JSON parsing instead of raw text regex so nested JSON does not inflate the result.
- **Operators can re-verify Loki-shipped rows.** Run
  `walter-os audit verify-chain --from-loki [date]` to query live Loki
  for one UTC audit day and re-run the local hash-chain verifier against
  the returned rows. Set `WALTER_AUDIT_LOKI_URL` to the Loki base URL or
  pass `--loki-url <url>` for a one-off check. If Loki is behind an auth
  proxy, set `WALTER_AUDIT_LOKI_TOKEN`; the verifier sends it as a bearer
  token and never prints it in success or failure output. Optional controls:
  `WALTER_AUDIT_LOKI_TIMEOUT_SECONDS` (default `10`, max `300`),
  `WALTER_AUDIT_LOKI_LIMIT` (default `5000`), and
  `WALTER_AUDIT_LOKI_QUERY` (default
