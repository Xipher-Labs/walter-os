...
| **promtail** | `/var/log` (ro), `${WALTER_AUDIT_DIR:-/home/walter/.config/walter-os/audit}` as `/var/log/walter-audit` (ro), `/var/lib/docker/containers` (ro), `/var/run/docker.sock` (mount flagged `:ro`) | **effective: read/write on the Docker API** (see below) | no | Tails every host log + every container log + Walter-OS audit-chain JSONL rows; uses the Docker socket to discover containers for log shipping to Loki. **Important caveat**: a `:ro` bind-mount on a Unix socket does not restrict the API surface — anything that connects to the socket can issue any Docker API call (start/stop/exec/destroy containers, mount host paths into new ones). Treat promtail's docker.sock access as full Docker daemon control + handle the container like an admin tool. |
...
- **Promtail also reads Walter-OS audit-chain rows.** The host path `${WALTER_AUDIT_DIR:-/home/walter/.config/walter-os/audit}` is mounted read-only at `/var/log/walter-audit`, and `promtail.yml` scrapes `/var/log/walter-audit/chain-*.jsonl` with static Loki labels `app=walter-os`, `kind=audit-chain`, and `host=${WALTER_AUDIT_HOST:-walter-vm}`. The bind uses `create_host_path: false`, so create the audit directory as the Walter operator before starting observability. Set `WALTER_AUDIT_DIR` before running this compose profile when Walter uses a non-default config directory; it should point at the host audit directory itself, not its parent. Set `WALTER_AUDIT_HOST` when several Walter hosts ship into the same Loki. Promtail parses the JSON row's `ts` field as the Loki timestamp, then forwards the JSONL line as-is; Grafana parses fields at query time with LogQL `| json`.
- **Grafana provisions the audit dashboard as `walter-audit`.** The dashboard uses the existing Loki datasource UID `loki` and starts from the low-cardinality selector `{app="walter-os", kind="audit-chain"}` for each panel.
