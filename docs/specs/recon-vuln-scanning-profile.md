...
Pattern reference: `mcp/servers.json` already splits into `default` (read-mostly, low-blast-radius) and `manual` (destructive, money-spending, lateral-movement-risk) profiles. The recon tooling lives in the same risk class.
...
| # | Decision | Why |
|---|---|---|
| D-1 | **New MCP profile: `pentest`** alongside `default` and `manual`. Lives in `~/.claude/settings.pentest.json` and `~/.codex/config.pentest.toml`. | Same pattern as the existing high-risk profile swap (`walter-os profile high-risk`). |
...
| D-3 | **Authorization gate ALWAYS required.** Every state-changing skill function refuses to run against a target unless the operator explicitly types the target hostname AND a one-line authorization note ("operator owns walter.example", "bug bounty program at hackerone.com/foo", etc.). | Legal cover. No accidental scan. The note gets logged. |
...
| D-6 | **`pentest` profile, when active, makes approval-gate stricter, not looser.** Every nuclei/httpx invocation is treated as a `requires-confirmation` action in `CATEGORY_MIN_TIER`. | The profile UNLOCKS the tooling but does NOT loosen the gate; reflexive guardrails remain. |
