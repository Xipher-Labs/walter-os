...
| # | Decision | Why |
|---|---|---|
| D-1 | **New settings profile: `pentest`** alongside `default` and `high-risk`. Lives in `~/.claude/settings.pentest.json` and `~/.codex/config.pentest.toml`. Activated via `walter-os profile pentest` — extends the existing `walter-os profile {default,high-risk}` swap pattern to a third profile name. This is future profile-generator work; the current installer does not ship `install.sh --profile-pentest`. | Same swap pattern, third tier. |
