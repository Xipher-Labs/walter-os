...
| # | Decision | Why |
|---|---|---|
| D-1 | **File Passing in v0.5.0**, immediately after the OSS Trust roadmap's GHSA partner-registration item ships. Silver in v0.6.0. (Naming note: the roadmap's per-layer item identifier `D-1` happens to share the letter with this spec's first decision identifier `D-1` — both unrelated. To disambiguate, references to the roadmap item use "GHSA item" or "Layer D D-1" explicitly throughout this spec; references to in-spec decisions stay as `D-N` without qualification.) | GHSA is one of Passing's required answers. Sequencing is forced. |
