...
**Depends on**: P1-09 env-allowlist parser — already on `main` via [PR #69](https://github.com/Xipher-Labs/walter-os/pull/69). The egress-loader (this spec's AC-1) is modeled on the same shape (text file, one-per-line, no eval, allowlist semantics). The reference is a CODE-PATTERN dependency: this spec doesn't introduce new env vars that need to land in the env-allowlist itself — the egress-loader is a sibling helper that mirrors the env-loader's design.
...
| D-7 | **CLI: `walter-os egress {add,remove,list,test,import}`**. `test <host>` returns whether the host would be allowed without making a request. `import <path>` overwrites the on-disk allowlist with the contents of `<path>` (used by `install.sh` to seed from the bundled example — AC-3 + AC-5 below). | Operator can audit + adjust without re-editing the file. |
...
- [ ] `contexts/_examples/egress-allowlist.example.txt` ships with these defaults (one per line):
  ...
  **Variable expansion**: `${WALTER_DOMAIN}` (and any other `${VAR}` reference) is expanded by `walter-os egress import` at import time via `envsubst`, NOT by the loader at match time. After import, the operator-on-disk allowlist contains the literal expanded hostnames (e.g. `llm.example.tld` rather than `llm.${WALTER_DOMAIN}`). If `WALTER_DOMAIN` is unset at import time, those lines are **skipped with a WARN on stderr** ("WARN: WALTER_DOMAIN unset; skipping 2 lines: llm.${WALTER_DOMAIN}, secrets.${WALTER_DOMAIN}") — not silent. The operator sees the message and either sets the var and re-imports OR keeps the partial import as-is. The loader itself is variable-unaware (matches literal strings + the D-5 wildcards only). AC-3 below pins this behavior for `walter-os egress import`.
