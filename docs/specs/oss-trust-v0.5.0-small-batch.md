...
Walter-OS ships a raw git hook for gitleaks (`hooks/pre-commit-gitleaks.sh`). Operators using the [pre-commit framework](https://pre-commit.com) for other repos have to choose between Walter-OS's raw hook OR their own framework setup — they conflict on the same git-hook path.
...
- [ ] `.pre-commit-config.yaml` (new) at repo root:
  ...
  # raw hook at hooks/pre-commit-gitleaks.sh). Choose ONE — running
  # both runs gitleaks twice per commit.
...
- [ ] `.github/SECURITY.md` symlink to top-level (some tools look in `.github/`).
- [ ] Issue template for security disclosures DOES NOT exist publicly — researchers go through advisories, not issues.
...
`@types/*` is a documented-safe scope (DefinitelyTyped curators + automated bot publishing). The allowlist is per-SCOPE not per-package, so a one-off `@types/malicious-package` published from a compromised account still goes through normal `check-release-age` AND `mcp-scanner` checks. Risk delta is negligible.
...
1. **C-3 pre-commit framework rev pinning**: pin to a specific gitleaks-action tag (proposal — same supply-chain discipline as elsewhere)? Or use `pre-commit autoupdate` flow? Proposal: pin; autoupdate is opt-in via operator running it deliberately.
