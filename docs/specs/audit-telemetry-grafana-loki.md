...
- [ ] Loki config (`setup/walter-host/services/observability/loki/loki-config.yml`) gains a labeled retention stanza for `app=walter-os, kind=audit-chain` using `WALTER_AUDIT_LOKI_RETENTION_DAYS` env var (default 30).
