...
- [ ] Row schema documented in `docs/operational/audit-chain-format.md`:
  ```json
  {
    ...
    "decision_source": "approval-gate" | "bash-denylist" | "branch-flow-guard" | "capability-check" | "network-gate" | "operator-confirm" | "pre-commit-tests",
...
- [ ] Every existing PreToolUse hook (`approval-gate.sh`, `bash-denylist.sh`, `network-gate.sh`, `branch-flow-guard.sh`, `pre-commit-tests.sh`, `capability-check.sh` when present) appends one row per decision to the chain (allow OR block).
