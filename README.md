...
> **Status — alpha (v0.7.0).** Things iterate fast. **Pin to a tag** in production.
...
> v0.7.0 resolves the audit-chain enforcement deadlock that could block the
> agent shell, adds a static local preview adapter, and hardens the release and
> Scorecard tooling. See the [CHANGELOG](CHANGELOG.md) for the full
> 0.6.2..0.7.0 batch.
...
```bash
walter-os upgrade --target v0.7.0
```
...
├── VERSION                      # Single-source semver (0.7.0)
