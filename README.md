...
Don't want to memorize steps? Walter-OS ships four copy-paste prompts you
hand to your agent (Claude Code, Codex CLI, or Cursor). The agent reads
the prompt, asks the minimum questions, writes the config files, and
verifies each step before moving on.

Pick the tier that matches what you actually want — they stack, so you
can start small and upgrade later without redoing earlier work.

| Tier | What you get | Time | Cost/mo | Prompt |
|---|---|---|---|---|
| **I — Agent contract** | `AGENTS.md` cascade + personal overlay + branch-flow + PR-title hooks + `walter-os` CLI | ~5 min | $0 | [`setup/agent-install/tier-1.md`](setup/agent-install/tier-1.md) |
| **II — + Local tooling** | Tier I + skills catalog (~50 skills) + MCP profiles (default + high-risk) + slash commands + `obra/superpowers` plugin | ~15 min | $0 | [`setup/agent-install/tier-2.md`](setup/agent-install/tier-2.md) |
...
| **IV — + Council + automation** | Tier III + 6-agent Walter Council with per-agent trust tiers + n8n workflows + Plane workspace structure + Control Tower dashboard | ~2–3 hrs | +~$10–50 LLM | [`setup/agent-install/tier-4.md`](setup/agent-install/tier-4.md) |

**How to use**: open your agent in any directory, paste the entire
fenced block from the chosen tier file, answer the questions one at a
time. The prompts are idempotent — re-paste to upgrade tiers, rotate
keys, or add things you skipped.
...
- Tier III's money-spending actions print the bill and require explicit
...
section below cover the same ground without an agent.
