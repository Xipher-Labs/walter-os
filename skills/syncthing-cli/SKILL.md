...
```bash
# Seed .stignore for a folder. Use `ssh ... -- tee <path>` (argv-style) rather
# than `ssh ... "cat > $path"` (string-style). The first passes the path as
# a separate SSH command word; the remote shell never re-parses it. This is
# the same P1-04 mitigation applied to the API helpers above — consistency
# matters because operators copy these examples verbatim.
