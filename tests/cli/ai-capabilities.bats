...
@test "walter ai configure writes matching model router preferences" {
  run bash "$WALTER_BIN" ai configure --profile claude-only
  [ "$status" -eq 0 ]

  prefs="${WALTER_CONFIG}/overlay/personal.env"
  [ -f "$prefs" ]
  grep -q "^WALTER_MODEL_BACKEND_REVIEW=claude$" "$prefs"
  grep -q "^WALTER_MODEL_FRONTEND=claude$" "$prefs"
  grep -q "^WALTER_MODEL_LONGFORM=claude$" "$prefs"
  grep -q "^WALTER_MODEL_QUICK_REFACTOR=claude$" "$prefs"
  grep -q "^WALTER_MODEL_BRAINSTORM=claude$" "$prefs"
  grep -q "^WALTER_MODEL_DEFAULT=claude$" "$prefs"
  grep -q "^WALTER_MODEL_PHI=local-ollama$" "$prefs"
}

@test "walter ai configure preserves unrelated personal.env keys" {
  mkdir -p "${WALTER_CONFIG}/overlay"
  cat >"${WALTER_CONFIG}/overlay/personal.env" <<'ENV'
WALTER_DOMAIN=example.test
GITHUB_USER=operator
WALTER_MODEL_BACKEND_REVIEW=old-model
ENV
  ...
  prefs="${WALTER_CONFIG}/overlay/personal.env"
  grep -q "^WALTER_DOMAIN=example.test" "$prefs"
  grep -q "^GITHUB_USER=operator" "$prefs"
  grep -q "^WALTER_MODEL_BACKEND_REVIEW=codex$" "$prefs"
  [ "$(grep -c "^WALTER_MODEL_BACKEND_REVIEW=" "$prefs")" -eq 1 ]
}

@test "walter ai configure keeps managed model header with regenerated block" {
  mkdir -p "${WALTER_CONFIG}/overlay"
  cat >"${WALTER_CONFIG}/overlay/personal.env" <<'ENV'
WALTER_DOMAIN=example.test
# === MODEL ROUTING ===
# Generated by: walter ai configure
# LiteLLM-style aliases. PHI must remain local.
WALTER_MODEL_BACKEND_REVIEW=old-model
UNRELATED_AFTER_HEADER=yes
ENV
  ...
  prefs="${WALTER_CONFIG}/overlay/personal.env"
  [ "$(grep -c "^# === MODEL ROUTING ===$" "$prefs")" -eq 1 ]
  header_line="$(grep -n "^# === MODEL ROUTING ===$" "$prefs" | cut -d: -f1)"
  unrelated_line="$(grep -n "^UNRELATED_AFTER_HEADER=yes$" "$prefs" | cut -d: -f1)"
  backend_line="$(grep -n "^WALTER_MODEL_BACKEND_REVIEW=codex$" "$prefs" | cut -d: -f1)"
  [ "$unrelated_line" -lt "$header_line" ]
  [ "$header_line" -lt "$backend_line" ]
  ! grep -q "^# LiteLLM-style aliases\\. PHI must remain local\\.$" "$prefs"
}

@test "walter ai configure mixed profile keeps brainstorm as Claude plus Codex" {
  ...
  grep -q "^WALTER_MODEL_BACKEND_REVIEW=codex$" "$prefs"
  grep -q "^WALTER_MODEL_FRONTEND=claude$" "$prefs"
  grep -q "^WALTER_MODEL_BRAINSTORM=claude,codex$" "$prefs"
  grep -q "^WALTER_MODEL_PHI=ollama$" "$prefs"
}
