...
assert_router_startup_model_probe_contract() {
  ...
  grep -q 'startupModelProbeFailure = message' "$server"
  grep -q 'console.error(`${message}: ${err.message}`)' "$server"
  grep -q "startupModelProbeFailure = startupModelProbeFailure || 'Startup model probe failed'" "$server"
  ! grep -q 'startupModelProbeFailure = .*err.message' "$server"
  ...
  grep -q "start_period: 30m" "$compose"
