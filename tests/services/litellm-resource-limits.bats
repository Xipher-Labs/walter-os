...
@test "litellm resource limits are overrideable by environment" {
  ...
  grep -q 'LITELLM_MEM_RESERVATION:-512m' "$COMPOSE_FILE"
  grep -q 'LITELLM_CPUS:-2.0' "$COMPOSE_FILE"
  grep -q 'LITELLM_DB_MEM_LIMIT:-1024m' "$COMPOSE_FILE"
  grep -q 'LITELLM_DB_MEM_RESERVATION:-512m' "$COMPOSE_FILE"
  grep -q 'LITELLM_DB_CPUS:-1.0' "$COMPOSE_FILE"
  grep -q 'LITELLM_PGBOUNCER_MEM_LIMIT:-128m' "$COMPOSE_FILE"
  grep -q 'LITELLM_PGBOUNCER_MEM_RESERVATION:-64m' "$COMPOSE_FILE"
  grep -q 'LITELLM_PGBOUNCER_CPUS:-0.25' "$COMPOSE_FILE"
}
