...
write_env_file() {
  ...
  # an `export VAR=...` directive that gets sourced by hooks/CLI/scripts.
  # A single quote in REPO_ROOT (`/tmp/foo'; touch X #`) would break out
  # of the previous literal `'${REPO_ROOT}'` form and inject commands
  # the next time the env file is sourced. printf '%q' produces a
  # bash-safe quoted form (`/tmp/foo\'\;\ touch\ X\ \#`) that re-sources
  # to exactly the original value, no injection.
  local q_repo_root
