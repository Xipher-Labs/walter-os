...
- **Audit P1-05 closed.** `hooks/approval-gate.sh` now hard-fails with
  ...
  is now wrapped in `set +u` … `set -u` so nounset cannot turn
  dashed keys into parameter-expansion surprises on supported Bash
  runtimes. `approval-gate.sh` requires Bash 4+ for associative arrays;
  macOS `/bin/bash` 3.2 is not a supported runtime for that hook.
