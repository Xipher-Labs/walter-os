...
> service stack (25+ services on a single VM, behind Cloudflare Access).
>
> **Time**: ~1–2 hours, plus ~15 min waiting on DNS propagation.
>
> **Costs money**: ~€25-50/month for a Hetzner CPX31–CPX41 VM, plus
> Cloudflare (free tier OK), plus a domain (one-time ~$10/yr).
>
> **Prereqs**:
> - Tier I + Tier II installed.
> - A Hetzner Cloud account with a project + API token (read-only for
>   first dry-run; write token minted later just for provisioning).
...
> - A working email at the domain you control (for CF Access OTP).
>
> **How to use**: paste the entire block between the `===` lines below.
> The agent walks you through provisioning and asks for tokens when
> needed.
>
> **STOP RULE**: Tier III spends money. The agent must show the bill in
> human-readable form before any state-changing Hetzner call, and the
> operator must confirm "yes" each time. See AGENTS.md "Money-spending
> MCPs" rules.
...
You are provisioning a Hetzner Cloud VM and bringing up the Walter-Host
service stack (25+ services) behind Cloudflare Access.

What gets deployed (core profile):
...
What this Tier does NOT include:
  - Council agents wired (Tier IV).
  - n8n workflows pre-loaded (Tier IV).
  - Plane workspace/projects created (Tier IV).
  - Control Tower running (Tier IV).

GROUND RULES — money + destructive
- Before ANY hcloud-cli call that creates/resizes/destroys, print the
  expected monthly cost delta and wait for explicit "yes" in chat.
- One Hetzner action per confirmation. "Spin up the VM" is NOT consent
  to also create 3 networks + a load balancer.
- Read-only HCLOUD_TOKEN first. Mint a write-scoped token only at the
  moment of provisioning. Revoke when done.
...
PRECHECK
  ...
  walter-os doctor                           # expect all ✓ from Tier I+II
  walter-os audit run --quick                # expect no critical findings
...
Ask:
  ...
  1.3  What email domain do you want allowed for CF Access logins?
       (Often the same as WALTER_DOMAIN; can be different — e.g. service
        on mycorp.dev, login as @mycorp.com.)
       → WALTER_AUTH_DOMAIN

  1.4  Login methods:
       a) OTP-to-email only (default, simplest, no Google Cloud setup)
...
STEP 2 — HETZNER VM PROVISIONING
  ...
  2.1  Hetzner Cloud read-only API token (project-scoped).
       Walter-OS validates the token + dry-runs the provisioning. NO
       money spent yet.
       → HCLOUD_TOKEN_READONLY

  2.2  Server type:
       Show the operator this comparison from setup/walter-host/README.md:
         CPX31 — 4 vCPU AMD, 8 GB RAM, 160 GB NVMe   — €15/mo
                 OK for: core profile, no comms/design/devrel
...
Compute the expected cost:
  €<server-price>/mo + €0.50/mo per public IPv4 + storage egress (usually
  €0). Print as: "Provisioning <type> in <location> will cost ~€<N>/mo."

Ask: "Confirm provisioning? (yes/no)"

If "yes":
  - Tell the operator to mint a WRITE-scoped Hetzner token now and paste it.
    → HCLOUD_TOKEN_WRITE  (used only for this step, revoked at the end)
  - Run hcloud-cli skill: provision VM with cloud-init script that:
      * installs docker + compose
      * adds operator's SSH public key (ask for it: ~/.ssh/id_*.pub)
...
an A or CNAME record pointing to the cloudflared tunnel (NOT to the VM
IP — keeps the VM IP private and gives DDoS protection):

Tunnel hostname pattern: `<sub>.${WALTER_DOMAIN}` → tunnel
...
Verify (DNS propagation ~1–5 min):
  for sub in home tower plane git grafana llm; do
    dig +short "$sub.$WALTER_DOMAIN" CNAME
  done
  # expect: all return *.cfargotunnel.com

================================================================================
STEP 4 — CLOUDFLARE ACCESS POLICIES

Walter-Host puts every service behind CF Access. No service is publicly
reachable; all require a successful OTP/Google login from an allowed
email domain.
...
This creates ~24 Access apps + policies. The script is idempotent — safe
to re-run.

Verify:
  for sub in home tower plane git grafana llm; do
    code=$(curl -sf -o /dev/null -m 5 -w '%{http_code}' "https://$sub.$WALTER_DOMAIN")
    echo "$sub: $code"
  done
  # expect: all return 302 (login redirect, NOT 200 which would mean
  # the service is publicly reachable — RED FLAG)
...
SSH to the VM:
  ...
  vi .env.local   # fill: WALTER_DOMAIN, CF_API_TOKEN, postgres passwords, etc.

Generate strong passwords:
  walter-os secrets generate    # writes to .env.local

Ask the operator to confirm .env.local looks right (this is also a good
moment to push the secrets to Infisical for backup once Infisical is up
— Step 6 covers that).
...
Optionally add profiles:
  docker compose --profile core --profile design --profile devrel up -d

Wait for all services to be healthy:
  while docker compose ps --format json | jq -r '.[].Health' | grep -qE 'starting|unhealthy'; do
    sleep 10
...
Once Infisical is up at https://secrets.${WALTER_DOMAIN}:
  ...
  7.1  Tell the operator to create their admin user via the UI (first
       login bootstrap, can't be automated for security reasons).
  7.2  Have them create a workspace `walter-vm-internal` and a project
       per service group.
  7.3  Have them create a Machine Identity called `walter-vm-prod` with
       Viewer access to walter-vm-internal + walter-shared, then paste
       the client_id + client_secret back into chat.
  7.4  Write the identity to /etc/walter-os/infisical-identity on the VM
       (chmod 600 root:root). This is what the Infisical sidecars use to
       fetch secrets per service at boot.
...
Final smoke test from the operator's machine:
  for sub in home tower plane git grafana llm secrets n8n; do
    code=$(curl -sf -o /dev/null -m 5 -w '%{http_code}' "https://$sub.$WALTER_DOMAIN")
    echo "$sub: $code"
  done
  # expect: all 302

Open https://home.${WALTER_DOMAIN} in a browser → CF Access login → after
auth, see the homepage dashboard with all service links.

If any service is unhealthy:
  ssh root@<vm-ip> 'cd /opt/walter-os && docker compose ps | grep -v healthy'
...
Print to the operator:
  ...
  Next steps:
    ...
    - Optional: add Tier IV for Council agents + n8n workflows + Control
      Tower (2–3 hours).
...
| Backups | Restic to Backblaze B2 if `RESTIC_REPOSITORY` configured (default off) |
...
Re-paste to:
- Add new profiles (e.g. you didn't enable `design`, now you want Penpot).
...
→ [Tier IV — Council + automation](tier-4.md) wires the 6-agent Council,
n8n workflows, Plane workspace structure, and Control Tower dashboard.
