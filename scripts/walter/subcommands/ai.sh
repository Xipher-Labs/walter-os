...
MODEL_PREFS_FILE="${WALTER_MODEL_PREFS_FILE:-${WALTER_CONFIG}/overlay/personal.env}"
...
Generated model-router preferences are written to:
...
route_to_model_pref() {
  local route
  route="$(normalize_provider_route "${1:-}")"
  case "$route" in
    ""|none) return 1 ;;
    *) printf '%s\n' "$route" ;;
  esac
...
join_unique_routes() {
  local joined="" route provider
  for route in "$@"; do
    route="$(normalize_provider_route "$route")"
    [[ -n "$route" && "$route" != "none" ]] || continue
    IFS=',' read -ra providers <<<"$route"
    for provider in "${providers[@]}"; do
...
strip_managed_model_prefs() {
  local file="$1" regex
  regex='^([[:space:]]*(export[[:space:]]+)?WALTER_MODEL_(BACKEND_REVIEW|FRONTEND|LONGFORM|QUICK_REFACTOR|PHI|BRAINSTORM|DEFAULT)=|# === MODEL ROUTING ===$|# Generated by: walter ai configure$|# LiteLLM-style aliases\. PHI must remain local\.$)'
  [[ -f "$file" ]] || return 0
  grep -Ev "$regex" "$file" || true
}

write_model_preferences() {
  local backend_review frontend longform quick_refactor phi brainstorm default_model tmp
  backend_review="$(route_to_model_pref "$route_infra_security_backend" || route_to_model_pref "$route_code_review" || printf '%s\n' codex)"
  frontend="$(route_to_model_pref "$route_ux_ui" || route_to_model_pref "$route_planning" || printf '%s\n' claude)"
  longform="$(route_to_model_pref "$route_planning" || route_to_model_pref "$route_ux_ui" || printf '%s\n' claude)"
  quick_refactor="$(route_to_model_pref "$route_infra_security_backend" || route_to_model_pref "$route_code_review" || printf '%s\n' codex)"
  phi="$(route_to_model_pref "$route_compliance_local_only" || printf '%s\n' local-ollama)"
  brainstorm="$(join_unique_routes "$route_planning" "$route_infra_security_backend" || route_to_model_pref "$route_research" || printf '%s\n' claude,codex)"
  default_model="$(route_to_model_pref "$route_planning" || route_to_model_pref "$route_ux_ui" || route_to_model_pref "$route_infra_security_backend" || printf '%s\n' claude)"

  mkdir -p "$(dirname "$MODEL_PREFS_FILE")"
  tmp="$(mktemp "${MODEL_PREFS_FILE}.tmp.XXXXXX")"
  strip_managed_model_prefs "$MODEL_PREFS_FILE" >"$tmp"

  if ! grep -q '^# === MODEL ROUTING ===$' "$tmp"; then
    if [[ -s "$tmp" ]]; then
      printf '\n' >>"$tmp"
    fi
    printf '%s\n' '# === MODEL ROUTING ===' >>"$tmp"
    printf '%s\n' '# Generated by: walter ai configure' >>"$tmp"
  fi

  {
    printf 'WALTER_MODEL_BACKEND_REVIEW=%s\n' "$backend_review"
    printf 'WALTER_MODEL_FRONTEND=%s\n' "$frontend"
    printf 'WALTER_MODEL_LONGFORM=%s\n' "$longform"
    printf 'WALTER_MODEL_QUICK_REFACTOR=%s\n' "$quick_refactor"
    printf 'WALTER_MODEL_PHI=%s\n' "$phi"
    printf 'WALTER_MODEL_BRAINSTORM=%s\n' "$brainstorm"
    printf 'WALTER_MODEL_DEFAULT=%s\n' "$default_model"
  } >>"$tmp"

  mv "$tmp" "$MODEL_PREFS_FILE"
  chmod 600 "$MODEL_PREFS_FILE"
}
...
cmd_configure() {
  ...
  echo "Model preferences written: $MODEL_PREFS_FILE"
